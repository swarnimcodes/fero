import { Visibility } from '@swarnimcodes/fero-lib/bindings/Visibility';
...
import { SendInfo } from '@swarnimcodes/fero-lib/bindings/SendInfo';
import { ChannelMessage } from '@swarnimcodes/fero-lib/bindings/ChannelMessage';
import { ChannelAction } from '@swarnimcodes/fero-lib';
...
async function getLatestVersion(vm: TauriVM) {
	try {
		const response = await fetch('https://api.github.com/repos/swarnimcodes/fero/releases/latest');
		if (!response.ok) {
