...
export default function ItemSide({ item }: Props) {
	const isError =
		item.state !== undefined &&
		['Cancelled', 'Rejected', 'Disconnected'].includes(item.state)

	return (
		<div>
			<div className="relative w-[62px] h-[62px]">
				{item.ack_bytes !== undefined && (
					<svg
						width="62"
						height="62"
						viewBox="0 0 250 250"
						className={`circular-progress${isError ? ' error' : ''}`}
						style={{ [getProgress(item)]: '' } as React.CSSProperties}
					>
						<circle className="bg" />
						<circle className="fg" />
					</svg>
				...
				<div className="h-14 w-14 rounded-full bg-white absolute top-0 left-0 bottom-0 right-0 m-auto">
					{item.state === 'Finished' ? (
						<svg
			...
			{/* Pin code */}
			{(item.state === 'WaitingForUserConsent' || item.state === 'SentIntroduction') &&
				item.pin_code && (
					<p className="text-center inline-flex gap-1 mt-4 text-sm items-center">
