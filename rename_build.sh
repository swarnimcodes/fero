...
for file in $files; do
    ...
    if [[ "$filename" =~ ([Ff]ero)_?([0-9]+\.[0-9]+\.[0-9]+)_(.*)\.${extension} ]]; then
        version="v${BASH_REMATCH[2]}"
        anything="${BASH_REMATCH[3]}"
    elif [[ "$filename" =~ ([Ff]ero)-([0-9]+\.[0-9]+\.[0-9]+)-([0-9]+)\.(.*)\.${extension} ]]; then
        version="v${BASH_REMATCH[2]}"
        anything="${BASH_REMATCH[3]}-${BASH_REMATCH[4]}"
    elif [[ "$filename" =~ ([Ff]ero)_?([0-9]+\.[0-9]+\.[0-9]+)_?(.*)\.${extension} ]]; then
        version="v${BASH_REMATCH[2]}"
        anything="${BASH_REMATCH[3]}"
    else
    ...
    if [ -n "$glib_ver" ]; then
        if [ -n "$debug_mode" ]; then
            new_filename="fero-${tauri_ver}-debug_${version}_glibc-${glib_ver}_${anything}.${extension}"
        else
            new_filename="fero-${tauri_ver}_${version}_glibc-${glib_ver}_${anything}.${extension}"
        fi
    else
        if [ -n "$debug_mode" ]; then
            new_filename="fero-${tauri_ver}-debug_${version}_${anything}.${extension}"
        else
            new_filename="fero-${tauri_ver}_${version}_${anything}.${extension}"
        fi
    fi
